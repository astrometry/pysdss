__all__ = ['dg_params', 'dg_apply', 'dg_psf', 'kl_at_points']

from astro.pyfits_utils import *
from numpy import *
from scipy.ndimage.filters import gaussian_filter

# Returns (a, sigma1, b, sigma2)
def dg_params(psfield, band):
...
def dg_apply(img, dgparams):
    (a, s1, b, s2) = dgparams
    a /= (s1**2 + b*s2**2)
    return a * (s1**2 * gaussian_filter(img, s1) +
            b * s2**2 * gaussian_filter(img, s2))

def dg_psf(dgparams, shape=(51,51)):
    img = zeros(shape)
    h,w = shape
    img[h/2, w/2] = 1.
    return dg_apply(img, dgparams)
